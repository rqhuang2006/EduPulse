...
def main() -> None:
    ensure_dirs()
    event = require_id_term(enrich_term(load_ods("exam_quiz")))
    if event.empty:
        ...
        term = event.groupby(["XH", "TERM_ID"], as_index=False).agg(
            FEATURE_EXAM_COUNT=("XH", "count"),
            FEATURE_EXAM_SCORE_AVG=("SCORE_NUM", "mean"),
            FEATURE_EXAM_MISSING_COUNT=("MISSING_FLAG", "sum"),
        )

    write_parquet(event, DWD_DIR / "study_l6_exam_quiz_event.parquet")
    write_parquet(term, DWD_DIR / "study_l6_exam_quiz_term.parquet")
    print(f"l6 exam quiz written: event={len(event)} term={len(term)}")
