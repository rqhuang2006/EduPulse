...
def build_domain_risks() -> Dict[str, Dict[str, Dict[str, object]]]:
    study_rows = read_csv_dicts(OUT_DIR / "predictions_test.csv")
    study = aggregate_mean_by_sid(study_rows, "pred_fail_proba", ["best_cls_model"])

    life_rows = read_csv_dicts(OUT_DIR / "life" / "predictions_test.csv")
    life = aggregate_mean_by_sid(life_rows, "pred_prob")

    sport_rows = read_csv_dicts(OUT_DIR / "sport" / "predictions_test.csv")
...
def build_master_records(shapmap_root: Path) -> Tuple[List[Dict[str, object]], List[Dict[str, object]]]:
    domain_risks = build_domain_risks()
    ...
    for row in records:
        ...
        top_behaviors = "、".join([value for value in [row.get("study_shap_top1"), row.get("life_shap_top1"), row.get("sport_shap_top1")] if value]) or "暂无显著行为特征"
        row["profile_text"] = (
...
def export_outputs(master: List[Dict[str, object]], shap_global: List[Dict[str, object]], out_dir: Path) -> None:
    ...
    dimension_comparison = [{"student_id": row["student_id"], "life_risk": round_or_none(row.get("life_risk")), "study_risk": round_or_none(row.get("study_risk")), "sport_risk": round_or_none(row.get("sport_risk")), "total_risk": round_or_none(row.get("total_risk"))} for row in master]
    ...
    risk_stats = [{"risk_level": key, "student_count": len(rows), "avg_total_risk": round_or_none(mean(row.get("total_risk") for row in rows)), "ratio": round(len(rows) / max(len(master), 1), 4)} for key, rows in sorted(risk_groups.items(), key=lambda item: len(item[1]), reverse=True)]
    ...
    dimension_stats = [{"dominant_dimension": key, "dimension_name": DOMAIN_CN.get(key, key), "student_count": len(rows), "avg_total_risk": round_or_none(mean(row.get("total_risk") for row in rows)), "ratio": round(len(rows) / max(len(master), 1), 4)} for key, rows in sorted(dimension_groups.items(), key=lambda item: len(item[1]), reverse=True)]
    ...
    map_summary = [{"dominant_MAP": key, "map_name": MAP_LABELS.get(key, key), "student_count": len(rows), "avg_M_score": round_or_none(mean(row.get("M_score") for row in rows)), "avg_A_score": round_or_none(mean(row.get("A_score") for row in rows)), "avg_P_score": round_or_none(mean(row.get("P_score") for row in rows)), "ratio": round(len(rows) / max(len(master), 1), 4)} for key, rows in sorted(map_groups.items(), key=lambda item: len(item[1]), reverse=True)]
    ...
    for key, rows in sorted(pattern_groups.items(), key=lambda item: len(item[1]), reverse=True):
        ...
        pattern_summary.append({"pattern_label": key, "student_count": len(rows), "ratio": round(len(rows) / max(len(master), 1), 4), "avg_total_risk": round_or_none(mean(row.get("total_risk") for row in rows)), "main_dimension": DOMAIN_CN.get(main_dimension, main_dimension), "main_MAP": MAP_LABELS.get(main_map, main_map)})
    ...
    student_profile = [{"student_id": row["student_id"], "risk_level": row["risk_level"], "dominant_dimension": DOMAIN_CN.get(str(row["dominant_dimension"]), "未知"), "pattern_label": row["pattern_label"], "profile_text": row["profile_text"]} for row in master]
    ...
    student_intervention = [{"student_id": row["student_id"], "intervention_type": row["intervention_type"], "intervention_text": row["intervention_text"], "priority": row["priority"]} for row in master]
    ...
    report_rows = [{"student_id": row["student_id"], "risk_level": row["risk_level"], "life_risk": round_or_none(row.get("life_risk")), "study_risk": round_or_none(row.get("study_risk")), "sport_risk": round_or_none(row.get("sport_risk")), "total_risk": round_or_none(row.get("total_risk")), "dominant_dimension": DOMAIN_CN.get(str(row["dominant_dimension"]), "未知"), "dominant_MAP": MAP_LABELS.get(str(row["dominant_MAP"]), str(row["dominant_MAP"])), "pattern_label": row["pattern_label"], "profile_text": row["profile_text"], "intervention_text": row["intervention_text"], "priority": row["priority"]} for row in master]
...
def main() -> None:
    args = parse_args()
    master, shap_global = build_master_records(args.shapmap_root)
    if not master:
